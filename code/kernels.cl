...
__kernel void naiveForwardReduction(__global double *a_d,
                               __global double *b_d,
                               __global double *c_d,
                               __global double *d_d,
                               int nx,
                               ...
                               int stride)
{
    ...
    else
    {
        if (i == (nx-1)) 
        {
            ...
            barrier(CLK_GLOBAL_MEM_FENCE);
            d_d[gi3d] = d_d[gi3d] - d_d[gi3d-stride/2]*k1;
        }
        else
        {
            k1 = a_d[i]/b_d[i-stride/2];
            ...
            barrier(CLK_GLOBAL_MEM_FENCE); 
            d_d[gi3d] = d_d[gi3d] - d_d[gi3d-stride/2]*k1 - d_d[gi3d+stride/2]*k2;
        }
    }
}

__kernel void naiveBackSubstitution(__global double *a_d,
                                   __global double *b_d,
                                   __global double *c_d,
                                   __global double *d_d,
                                   int nx,
                                   ...
                                   int stride)
{
    ...
    if (i < stride)
    {
        d_d[gi3d] = (d_d[gi3d] - c_d[i]*d_d[gi3d+stride/2])/b_d[i];
    }
    else
    {
        d_d[gi3d] = (d_d[gi3d] - a_d[i]*d_d[gi3d-stride/2] - c_d[i]*d_d[gi3d+stride/2])/b_d[i];
    }
