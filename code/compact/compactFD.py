...
class CompactFiniteDifferenceSolver:
    ...
    def dfdx(self, f_global, dx, x_global, f_local, f_g, x_g, print_timings=False):
        ...
        bz = 1
        by = 1
        #evt = self.prg.MultiNCyclicReduction(self.queue, [nx, ny, nz], [nx, by, bz],
        #    a_g, b_g, c_g, x_g, np.int32(nx), np.int32(ny), np.int32(nz), np.int32(nx), np.int32(by),
        #        cl.LocalMemory(nx*by*bz*8), cl.LocalMemory(nx*by*bz*8), cl.LocalMemory(nx*by*bz*8), cl.LocalMemory(nx*by*bz*8)) 
        #evt.wait()

        self.block_solver.solve(x_g, [bz, by])
